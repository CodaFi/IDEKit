...
- (NSAttributedString *) collapsedTextInAttachmentFixIcons: (BOOL) fixIcons
{
    ...
    if (collapsedData) {
		...
		if (fixIcons && [uncollapsedText containsAttachments]) {
			...
			for (NSUInteger i=0;i<[uncollapsedText length];) {
				NSRange range;
...
- (NSInteger) uncollapseFromAttachment: (IDEKit_FoldedTextAttachment *)attachment atIndex: (NSUInteger) offset selectResult: (BOOL) shouldSelect
{
...
- (IBAction) deepUncollapse:(id) sender
{
    ...
    for (NSUInteger i=0;i<[(NSString *)workingCopy length];) {
...
- (void) megaFoldSelection: (NSRange) range
{
    ...
    NSUInteger pointOfFolding = range.location + range.length;
...
//- (BOOL)textView:(NSTextView *)textView clickedOnLink:(id)link atIndex:(NSUInteger)charIndex	// Delegate only.
//{
...
- (NSUInteger) foldedLocation: (NSUInteger) offset // convert from unfolded to folded
{
...
- (NSAttributedString *)collapsedTextAtIndex: (NSUInteger) offset // this may be collapsed, so call uncollapsedString on it
{
    ...
    if ([[self string] characterAtIndex: offset] == NSAttachmentCharacter) {
		IDEKit_FoldedTextAttachment *attachment = [self attribute: NSAttachmentAttributeName atIndex: offset effectiveRange: NULL];
