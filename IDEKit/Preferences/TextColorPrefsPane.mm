...
{
    return [NSArray arrayWithObjects: IDEKit_TextColorsPrefKey,IDEKit_TextColorDefaultBrowserKey,IDEKit_TextColorDefaultStateKey, NULL];
}
...
{
    [myDefaults setObject: [NSNumber numberWithInt: [[sender selectedCell] tag]] forKey: IDEKit_TextColorDefaultStateKey];
}
-(IBAction) changeColor: (id) sender
{
	...
	[newValue setObject: [[sender color] htmlString] forKey: colorName];
	[myDefaults setObject: newValue forKey: IDEKit_TextColorsPrefKey];
    } else {
	NSLog(@"Couldn't find name for color %d",[sender tag]-100);
    }
}
